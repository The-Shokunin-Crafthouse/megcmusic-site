...
TITLES SHE ACTUALLY PICKED BEFORE — `chose` is the one she used, `passedOver` are the options she was offered alongside it and declined:
{{CHOSEN_TITLES}}
...
- 3–5 title options, each with a one-sentence rationale tied to her data or the playbook rules (say which). Weight them toward what her past picks above have in common — everything in `passedOver` was already good enough to offer and she still declined it, so that contrast is real preference data. **If that list is empty, ignore it** rather than inventing a pattern from no data.
